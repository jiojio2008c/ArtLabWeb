  ...
  function index (...args) {
        ...
        detach(async function () {
          ...
          if (item.type === "END") {
            ...
            const repeat =
              state.concurrency === -1 ?
                state.stack.length
              : state.concurrency - state.running;
          ...
          try {
            ...
            const result =
              ...
              : typeof item.handler === "function" ? await item.handler.call()
              : await item.handler;
            const position = state.stack_running
              .map((i) => (i === item ? i : -1))
              .filter((i) => i !== -1);
      ...
      return catcher(
        new Promise(function (resolve, reject) {
          ...
          if (Array.isArray(items)) {
            Promise.all(items.map((item) => all(item))).then(resolve, reject);
          } else {
            state.stack.push({
              type: "ITEM",
              handler: items,
    ...
    const wrap = (promise, fluent) => {
      ...
      promise.call = function (items) {
        if (state.closed) {
          return Promise.reject(
            Error("EACH_CLOSED: cannot schedule new items when closed."),
          );
        }

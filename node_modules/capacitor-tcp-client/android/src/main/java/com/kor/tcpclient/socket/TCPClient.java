package com.kor.tcpclient.socket;
...
@NativePlugin(permissions = { Manifest.permission.ACCESS_NETWORK_STATE })
public class TCPClient extends Plugin {

    private Socket socket;
    private DataOutputStream mBufferOut;
    private DataInputStream mBufferInput;
    ...
    @PluginMethod
    public void onData(PluginCall call) {
        final Integer client = call.getInt("client");
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                try {
                    final Socket socket = clients.get(client);
                    while (true) {
                        mBufferInput = new DataInputStream(new BufferedInputStream(socket.getInputStream()));
                        if (mBufferInput != null) {
                            String message = mBufferInput.readUTF();
                            JSObject ret = new JSObject();
                            ret.put("success", true);

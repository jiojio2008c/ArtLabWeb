...
const ArtworkLaunchTransition: React.FC<ArtworkLaunchTransitionProps> = ({
  ...
  const returnCompleteRef = useRef(onReturnComplete)
  ...
  useEffect(() => {
    ...
    returnCompleteRef.current = onReturnComplete
  ...
  useLayoutEffect(() => {
    ...
    const adjustment = source?.closest<HTMLElement>('.upload-screen') ?? source
    const starElements = starsRef.current.filter((star): star is HTMLSpanElement => Boolean(star))

    if (!root || !veil || !artwork || !image || !burst || !source || !adjustment) {
      if (returningRef.current) returnCompleteRef.current()
    ...
    const centeredScale = Math.max(0.38, Math.min(0.62, centeredWidth / Math.max(1, sourceRect.width)))
    ...
    gsap.set(starElements, { opacity: 0, scale: 0, transformOrigin: '50% 50%', force3D: true })
    ...
    const start = async () => {
      await Promise.all([
        waitForImageElement(image, 700),
        mask ? waitForImageElement(mask, 700) : Promise.resolve()
      ])
      if (cancelled) return
    ...
    return () => {
      ...
      timeline.kill()
      gsap.killTweensOf([root, veil, artwork, image, mask, burst, adjustment, ...starElements].filter(Boolean))
      gsap.set(adjustment, { clearProps: 'opacity,transform,filter' })
    }
  }, [accent, imageMode, imageTransform, maskUrl, secondary, sourceRef, sourceUrl, stars])

  useEffect(() => {
    returningRef.current = returning
    if (!returning || returnStartedRef.current) return
    ...
    if (!root || !veil || !artwork || !adjustment || !timeline) {
      returnCompleteRef.current()
      return
    }

    timeline.pause()
    gsap.killTweensOf([veil, artwork, burst, adjustment, ...starElements].filter(Boolean))
    gsap.to(starElements, { opacity: 0, duration: 0.1, overwrite: true })
    gsap.to(burst, { opacity: 0, duration: 0.1, overwrite: true })
    gsap.to(artwork, {
      x: 0,
      y: 0,
      scale: 1,
    ...
    <div ref={rootRef} className="artwork-launch-transition" aria-hidden="true">
      ...
      <div className="artwork-launch-stars">
        {stars.map((star, index) => (
          <span
            ...
            style={{ '--star-color': star.color } as React.CSSProperties}
          />
        ))}
      </div>
      ...
      <div ref={artworkRef} className="artwork-launch-artwork">
        <div className="artwork-launch-frame">
          <img
            ref={imageRef}
            className={`artwork-launch-image is-${imageMode}`}
            src={sourceUrl}
            style={imageMode === 'positioned' ? { transform: imageTransform } : undefined}
            alt=""
          />
          {maskUrl && <img ref={maskRef} className="artwork-launch-mask" src={maskUrl} alt="" />}

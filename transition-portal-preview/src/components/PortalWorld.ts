...
import { portalFragmentShader, portalVertexShader } from '../shaders/portalShaders.ts'
...
export const createPortalWorld = (canvas: HTMLCanvasElement, origin: PortalOrigin): PortalWorld => {
  ...
  const glowPlane = new THREE.Mesh(new THREE.PlaneGeometry(3.3, 3.75), glowMaterial)
  glowPlane.position.z = 0.03
  portalGroup.add(glowPlane)

  const boxGeometry = new THREE.BoxGeometry(2.32, 2.64, 0.76)
  ...
  const render = () => {
    ...
    const settle = range(progress, 0.86, 1)
    const visibility = 1 - settle
    ...
    portalGroup.rotation.x = THREE.MathUtils.lerp(-0.03, 0.08, travel) + Math.sin(elapsed * 1.5) * 0.018
    portalGroup.rotation.y = THREE.MathUtils.lerp(-0.18, 0.02, travel) + Math.sin(elapsed * 1.2) * 0.025
    portalGroup.rotation.z = Math.sin(elapsed * 1.7) * 0.012 * activation
    ...
    for (let index = 0; index < particleCount; index += 1) {
      ...
      const baseX = particleData.basePositions[offset]
      const baseY = particleData.basePositions[offset + 1]
      const baseZ = particleData.basePositions[offset + 2]
      const radialPush = 1 + penetration * (1.5 + seed * 1.9)
      positionAttribute.array[offset] = baseX * radialPush
      positionAttribute.array[offset + 1] = baseY * (1 + penetration * 0.72)
      positionAttribute.array[offset + 2] = baseZ + ((elapsed * (0.45 + seed) + progress * 6.5) % 9)
    }
    ...
    trails.rotation.z = Math.sin(elapsed * 0.65) * 0.035
    ...
    wireLandscape.rotation.y = penetration * 0.16
